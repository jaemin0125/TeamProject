...
public class PlayerService {
	...
	public void spawnApplePeriodically() {
		long currentAppleCount = sceneObjects.values().stream().filter(obj -> "apple".equals(obj.getItemType()))
				.count();
	...
	public void spawnGunPeriodically() {
		long currentGunCount = sceneObjects.values().stream().filter(obj -> "ak-47".equals(obj.getItemType())).count();
	...
	public void spawnPipePeriodically() {
		long currentPipeCount = sceneObjects.values().stream().filter(obj -> "pipe".equals(obj.getItemType())).count();
	...
	private void spawnNewItem(String itemType, String modelPath, String objectType, double yPos, boolean stackable) {
		...
		ObjectState newItem = new ObjectState(itemId, randomPosition, objectType, modelPath, itemType, null, stackable); // ammo는
																															// null로
																															// 초기화
	...
	public void registerPlayer(PlayerState playerState, String sessionId) {
		...
		logger.info("Registering player at position: x={}, y={}, z={}", playerState.getPosition().getX(),
				playerState.getPosition().getY(), playerState.getPosition().getZ());
		logger.info("Player registered/updated: ID={}, Nickname={}, SessionID={}", playerState.getId(),
				playerState.getNickname(), sessionId);
	 ...
	 */
	public void updatePlayerState(String playerId, Position newPosition, double newRotationY,
		...
		if (player != null) {
			...
			// logger.debug("Player {} updated position to ({}, {}, {})", playerId,
	...
	public boolean pickUpItemFromScene(String playerId, String objectId,
			com.example.demo.dto.ItemActionRequest.ItemData itemData) {
		PlayerState player = connectedPlayers.get(playerId);
		...
		logger.info("Scene object {} (type: {}) removed from scene by player {}.", objectId, sceneObject.getItemType(),
				playerId);
		...
		if (addItemToPlayerInventory(playerId, pickedItem)) {
			logger.info("Player {} picked up item {} (type: {}).", playerId, pickedItem.getName(),
					pickedItem.getType());
			return true;
		} else {
	...
	private Item convertObjectStateToItem(ObjectState sceneObject,
			com.example.demo.dto.ItemActionRequest.ItemData itemData) {
		...
		return new Item(sceneObject.getId(), itemType, sceneObject.getObjectType(), 1, imagePath, 10, currentAmmo,
				reserveAmmo, stackable);
	}
	...
	public boolean addItemToPlayerInventory(String playerId, Item itemToAdd) {
		...
		if (itemToAdd.isStackable()) {
			Optional<Item> existingItemOpt = player.getInventory().stream()
					.filter(i -> i.getName().equals(itemToAdd.getName())).findFirst();

			if (existingItemOpt.isPresent()) {
				Item existingItem = existingItemOpt.get();
				existingItem.setCount(existingItem.getCount() + itemToAdd.getCount());
				logger.info("Player {} already has stackable item {}. Increased count to {}.", playerId,
						existingItem.getName(), existingItem.getCount());
				return true; // 추가 성공
			}
		}
		...
		logger.info("Player {} added new item {} (ID: {}, Count: {}).", playerId, itemToAdd.getName(),
				itemToAdd.getId(), itemToAdd.getCount());
		return true;
	}
	...
	public boolean useItemFromPlayerInventory(String playerId, String itemId, int quantityToUse) {
		...
		Optional<Item> itemToUseOpt = player.getInventory().stream().filter(i -> i.getId().equals(itemId)).findFirst();
		...
		if (itemToUse.getCount() < quantityToUse) {
			logger.warn("Player {} only has {} of item {} but tried to use {}.", playerId, itemToUse.getCount(), itemId,
					quantityToUse);
			return false;
		}

		itemToUse.setCount(itemToUse.getCount() - quantityToUse);
		logger.info("Player {} used {} of item {}. Remaining count: {}.", playerId, quantityToUse, itemToUse.getName(),
				itemToUse.getCount());
		...
		if ("consumable".equalsIgnoreCase(itemToUse.getType()) || "food".equalsIgnoreCase(itemToUse.getType())) {
			...
			logger.info("Player {} used {}. Health restored from {} to {}.", playerId, itemToUse.getName(),
	 ...
	 */
	public boolean dropItemFromInventory(String playerId, String itemId,
			com.example.demo.dto.ItemActionRequest.ItemData itemData, Position dropPosition, int quantity) {
		PlayerState player = connectedPlayers.get(playerId);
		if (player == null) {
		...
		Optional<Item> itemToDropOpt = player.getInventory().stream().filter(i -> i.getId().equals(itemId)).findFirst();
		...
		if (itemToDrop.getCount() < quantity) {
			logger.warn("Player {} only has {} of item {} but tried to drop {}.", playerId, itemToDrop.getCount(),
					itemId, quantity);
			return false;
		}

		itemToDrop.setCount(itemToDrop.getCount() - quantity);
		logger.info("Player {} dropped {} of item {}. Remaining count: {}.", playerId, quantity, itemToDrop.getName(),
				itemToDrop.getCount());
		...
		ObjectState droppedObject = new ObjectState(newObjectId, dropPosition, itemToDrop.getName(), // objectType
				itemToDrop.getImage().replace(".png", ".glb"), // modelPath (png -> glb)
				itemToDrop.getName(), // itemType
				itemToDrop.getCurrentAmmo() > 0 || itemToDrop.getReserveAmmo() > 0
						? new Ammo(itemToDrop.getCurrentAmmo(), itemToDrop.getReserveAmmo())
						: null, // 탄약 정보
	...
	public boolean setPlayerCoin(String playerId, int newAmount) {
		...
		// 동기화 메시지 전송
		messagingTemplate.convertAndSend("/topic/hud/" + playerId, Map.of("type", "COIN_UPDATE", "coin", newAmount));
	...
	public boolean buyItemForPlayer(String playerId, String itemName, int price) {
		...
		if (itemName.equals("파이프")) {
			...
			List<Item> inventory = player.getInventory();
			boolean alreadyOwned = inventory.stream().anyMatch(item -> item.getName().equals("ak-47"));
			if (alreadyOwned) {
				System.out.println("❌ 이미 보유중인 아이템: 구매 불가");
				messagingTemplate.convertAndSend("/topic/hud/" + playerId, Map.of("type", "ITEM_GIVE_RESULT", "success",
						false, "message", itemName + " 은(는) 1개만 소지할 수 있습니다."));
		...
		if ("apple".equals(engItemName)) {
			item = new Item(UUID.randomUUID().toString(), "apple", "food", 1, "/objects/apple.png", 20, 0, 0, true,
					null);
		} else if ("pipe".equals(engItemName)) {
			item = new Item(UUID.randomUUID().toString(), "pipe", "tool", 1, "/objects/pipe.png", 0.0, 0, 0, false,
					null);
		} else if ("ak-47".equals(engItemName)) {
			item = new Item(UUID.randomUUID().toString(), "ak-47", "weapon", 1, "/objects/ak-47.png", 0.0, 30, 120,
					false, new Ammo(30, 120));
		}
...
//        if (item == null) return false;
		...
		if (success) {
			messagingTemplate.convertAndSend("/topic/inventory/" + playerId, getInventoryForPlayer(playerId));
		}
	 ...
	 * 
	 * @param playerId 사망 처리할 플레이어의 ID 신규 : 사망시 플레이어의 코인을 죽인 플레이어에게 모두 전송하고 업데이트합니다
	 */
	public void setPlayerDead(String playerId, String killerId) {
		...
		if (player != null) {
			...
			// 죽은 플레이어 HUD 업데이트
			messagingTemplate.convertAndSend("/topic/hud/" + playerId, Map.of("type", "COIN_UPDATE", "coin", 0));
		}
	}
